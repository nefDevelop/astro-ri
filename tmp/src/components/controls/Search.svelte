           ...
           rounded-xl text-lg px-3 py-2 hover:bg-(--btn-plain-bg-hover) active:bg-(--btn-plain-bg-active)"
            >
                ...
                {#if item.excerpt.includes("<mark>")}
                    ...
                    >
                        <span
                            style="display: inline-block; background-color: var(--btn-plain-bg-hover); color: var(--primary); padding: 0.1em 0.4em; border-radius: 5px; font-size: 0.75em; font-weight: 600; margin-right: 0.5em; shrink: 0;"
                        >
...
<style>
    ...
    .search-panel {
        max-height: calc(
            100vh - 100px
        ); /* Ajustar la altura máxima del panel de búsqueda */
        overflow-y: auto;
    }
</style>
